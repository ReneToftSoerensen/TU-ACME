# TU-ACME — Copilot repository instructions

TU-ACME is a Windows-only, interactive terminal UI over **Posh-ACME 4.x** for
...
- Module under `/src` (`TU-ACME.psd1`/`.psm1`, `Public/`, `Private/`); entry point
  `Start-TUACME`. Headless runner `PoshAcme-Renew.ps1` at repo root reuses
  `Private/Deploy.ps1` helpers. Tests in `/tests` (Pester 5).

## Build / test / run

- Import: `Import-Module ./src/TU-ACME.psd1 -Force`
- Run: `Start-TUACME`
- Lint: `Invoke-ScriptAnalyzer -Path ./src -Recurse -Settings ./PSScriptAnalyzerSettings.psd1` (must be clean)
- Test: `Invoke-Pester ./tests`

## Code navigation — search first, read second

A symbol index is generated at `.github/symbol-index.txt` by the
`copilot-setup-steps` workflow. Before opening files with `view`, use faster
search tools:

- **Find a function**: `grep "function Invoke-TUACMERenewal" .github/symbol-index.txt`
  or `grep -r "function Invoke-TUACMERenewal" src/`
- **Find all callers**: `grep -r "Submit-Renewal" src/` — one call, all files.
- **Find parameter usage**: `grep -r "RenewalDaysBefore" src/`
- **Cross-file questions**: prefer `grep` or the `search_code_subagent` tool over
  opening files one by one with `view`.

Use `view` only once you have already identified the exact file and line range
via search. This avoids reading large files in full when only a few lines matter.
...
- Do not add `CONTRIBUTING`, `CODE_OF_CONDUCT`, badges, or boilerplate unless
  asked.
- Do not use the old name "PoshTui" or "PoshTUI" anywhere — that was a throwaway
  internal test-app name. All public names use `TU-ACME` / `TUACME`.
