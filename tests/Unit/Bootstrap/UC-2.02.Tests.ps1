...
Describe 'UC-2.02 — Use-TUACMEStagingAccount switches server and account' -Tag 'Unit' {
    ...
    It 'calls Set-PAServer with staging URL then Set-PAAccount with staging ID' {
        InModuleScope TU-ACME {
            ...
            Assert-MockCalled Set-PAServer  -ParameterFilter { $DirectoryUrl -eq 'https://acme-staging.corp.local/directory' } -Times 1 -Exactly -Scope It
            Assert-MockCalled Set-PAAccount -ParameterFilter { $ID -eq 'stag-acct-001' } -Times 1 -Exactly -Scope It
        }
    }
}
