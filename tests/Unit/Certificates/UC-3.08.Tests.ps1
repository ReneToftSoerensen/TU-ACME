...
Describe 'UC-3.08 — Order writes Event 1003 on success' -Tag 'Unit' {
    BeforeAll {
        ...
        $env:ProgramData = Join-Path ([System.IO.Path]::GetTempPath()) ("TU-ACME-uc308-" + [guid]::NewGuid().ToString('N'))
    ...
    It 'writes Event 1003 with the new thumbprint on success' {
